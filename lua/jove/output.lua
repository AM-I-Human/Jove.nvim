...
local line_marks = {}
local line_input_signs = {}
local SIGN_GROUP = "jove_input_group" -- A dedicated sign group for our input prompts

--- Clears any previous input prompt sign for a given buffer and line.
local function clear_previous_input_prompt(bufnr, row)
	if line_input_signs[bufnr] and line_input_signs[bufnr][row] then
		local sign_id = line_input_signs[bufnr][row]
		-- Use pcall for safety, in case the sign was already removed
		pcall(vim.fn.sign_unplace, SIGN_GROUP, { id = sign_id })
		line_input_signs[bufnr][row] = nil
	end
end
...
function M.render_input_prompt(bufnr, row, jupyter_msg)
	...
	clear_previous_input_prompt(bufnr, row)

	-- Also clear any old output from a previous run on this line.
	clear_previous_output(bufnr, row)

	local sign_name = "JoveIn" .. tostring(exec_count)
	local sign_text = string.format("In[%d]:", exec_count)

	-- Define the sign's appearance. It's okay to redefine this.
	vim.fn.sign_define(sign_name, { text = sign_text, texthl = "Question" })

	-- Place the sign. lnum is 1-based. A new ID is generated by passing 0.
	local sign_id = vim.fn.sign_place(0, SIGN_GROUP, sign_name, bufnr, { lnum = row + 1 })

	if sign_id > 0 then
		if not line_input_signs[bufnr] then
			line_input_signs[bufnr] = {}
		end
		line_input_signs[bufnr][row] = sign_id
	end
end
