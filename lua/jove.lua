...
if current_file_path and current_file_path ~= "" and current_file_path ~= "<sfile>:p" then
    ...
    if path_info_fallback and path_info_fallback.source and path_info_fallback.source:sub(1,1) == "@" then
        ...
        vim.notify("[Jove] CRITICO: Impossibile determinare il percorso radice del plugin. <sfile> ha restituito: " .. current_file_path .. ", debug.getinfo().source: " .. vim.inspect(path_info_fallback and path_info_fallback.source or "nil"), vim.log.levels.ERROR)
    end
end
